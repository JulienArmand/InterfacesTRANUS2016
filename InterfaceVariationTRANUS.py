...
class InterfaceVariationTRANUS(QtGui.QMainWindow, InterfaceVariationTRANUSUI.Ui_MainWindow):
    ...
    def __init__(self, filepath):
        ...
        #We check if the output directory exist, and we create it if it doesn't.
        nameOutputDirectoryGlobal = "INTERFACE_OUTPUT"
        pathOutputDirectoryGlobal = os.path.join(self.stockTranusConfig.workingDirectory, nameOutputDirectoryGlobal)
        print pathOutputDirectoryGlobal
        if not os.path.exists(pathOutputDirectoryGlobal):
            os.makedirs(pathOutputDirectoryGlobal)
        
        #Creation of the output directory for this instance of the interface, with name based on date and time in the following format : d.m.Y.H.M.S
        self.nameOutputDirectoryInstance = time.strftime("%d.%m.%Y")+"."+time.strftime("%H.%M.%S")
        self.pathOutputDirectoryInstance = os.path.join(pathOutputDirectoryGlobal, self.nameOutputDirectoryInstance)
    ...
    def launch21(self):
        ...
        if(self.MinValue2_1.value() > self.MaxValue2_1.value()):
            ...
            for i in range(nbIterations):  
                ...
                pathScenarioResultDirectory = os.path.join(self.pathOutputDirectoryInstance, name)
                ...
                if(self.optionFreeze2_1.isChecked()):
                    print("Execution of Lcal with option freeze enabled")
                    interface.runLcal(True)
                else:
                    print("Execution of Lcal with option freeze disabled")
                    interface.runLcal(False)
                
                nameFile = "outputImploc" #This is the name of the temporary file holding the data generated by imploc in the project's main directory
                interface.runImplocVariationSubDir("J", nameFile)
                #Because of the limit of size for the path of imploc's result, we need to copy the data generated with pandas to the result directory
        ...
        if(not self.check22) :
            ...
            for i in range(nbIterations):  
                ...
                if not os.path.exists(pathScenarioResultDirectory):
                    os.makedirs(pathScenarioResultDirectory)
                interface = LcalInterface(self.stockTranusConfig,pathScenarioResultDirectory)
                if(self.optionFreeze2_2.isChecked()):
                    print("Execution of Lcal with option freeze enabled")
                    interface.runLcal(True)
                else:
                    print("Execution of Lcal with option freeze disabled")
        ...
        if(not self.check23) :
            ...
            for i in range(nbIterations):  
                ...
                if not os.path.exists(pathScenarioResultDirectory):
                    os.makedirs(pathScenarioResultDirectory)
                interface = LcalInterface(self.stockTranusConfig,pathScenarioResultDirectory)
                if(self.optionFreeze2_3.isChecked()):
                    print("Execution of Lcal with option freeze enabled")
                    interface.runLcal(True)
                else:
                    print("Execution of Lcal with option freeze disabled")
    ...
    def launch32(self):
        ...
        for i in range(nbIterations):  
                ...
                if not os.path.exists(pathScenarioResultDirectory):
                    os.makedirs(pathScenarioResultDirectory)
                interface = LcalInterface(self.stockTranusConfig,pathScenarioResultDirectory)
                if(self.optionFreeze3_2.isChecked()):
                    print("Execution of Lcal with option freeze enabled")
                    interface.runLcal(True)
                else:
                    print("Execution of Lcal with option freeze disabled")
        ...
        if(self.generate2_1):
            ...
            if(self.windowAvailable):
                ...
                for i in range(nbIter):
                    ...
                    pathToDirectory = os.path.join(self.pathOutputDirectoryInstance, nameDirectory)
